...
log6_breaks <- function(x, n = 3){ x <- x[is.finite(x) & x > 0]; if(!length(x)) return(NULL); rng <- range(x); 10^seq(log10(rng[1]), log10(rng[2]), length.out = n) }
...
build_plot <- function(d, title_text, subtitle_text, show_legend = TRUE){
  ...
  if (use_log){
    ...
    x_scale  <- scale_x_log10(breaks = log6_breaks(d$OR, n=3), labels = label_plain, limits = x_limits)
    ...
    x_scale  <- scale_x_continuous(limits = xr*c(0.95,1.05),
                                   breaks = scales::pretty_breaks(n=3), labels = label_plain)
...
if (nrow(df_non) > 0) {
  build_plot(
    df_non,
    title_text    = "Exposure: Dermatological disorders | Outcome: Type 2 Diabetes",
    subtitle_text = "Causal Effects of Dermatological disorderss on Type 2 Diabetes",
    show_legend   = TRUE
  )
} else {
...
if (nrow(df_t2d) > 0) {
  build_plot(
    df_t2d,
    title_text    = "Exposure: Type 2 Diabetes | Outcomes: Dermatological disorders",
    subtitle_text = "Causal Effects of Type 2 Diabetes on Dermatological disorderss",
    show_legend   = TRUE
  )
} else {
